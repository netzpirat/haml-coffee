(function() {
  ...
  module.exports = Haml = (function() {
    ...
    Haml.prototype.parseTag = function(exp) {
      var classes, doctype, haml, id, ids, klass, tag, text, tokens, _ref;
      try {
        ...
        haml = tokens[1];
        if (tokens[3] && !tokens[3].match(/^=/)) {
          text = tokens[3].replace(/^ /, '');
        }
        if (tokens[2]) {
          if (tokens[2].indexOf('>') !== -1) this.wsRemoval.around = true;
    ...
    Haml.prototype.parseOptions = function(exp) {
      var hasAttributes, pairs, _ref;
      _ref = this.parseAttributes(exp), pairs = _ref[0], hasAttributes = _ref[1];
      return {
        pairs: pairs,
        assignment: this.parseAssignment(exp, hasAttributes)
      };
    };

    Haml.prototype.parseAssignment = function(exp, hasAttributes) {
      var assignment;
      if (hasAttributes) {
      ...
      console.log(exp, assignment != null ? assignment[1] : void 0);
      if (assignment) {
        return assignment[1];
      } else {
        return;
    ...
    Haml.prototype.parseAttributes = function(exp) {
      ...
      if (!attributes) return [pairs, false];
      ...
      if (tokens.pairs.length > 0) {
        ...
        for (_j = 0, _len2 = _ref2.length; _j < _len2; _j++) {
          ...
          if (pair.key !== pair.value || this.format !== 'html5') {
            tagParts.push("" + pair.key + "=" + (this.quoteAttributeValue(pair.value)));
          } else {
