(function() {
  ...
  exports.run = function() {
    var compiled_output, compiler_options, input_filename, output_filename, template_name;
    input_filename = argv.i;
    output_filename = argv.o;
    template_name = argv.n;
    compiled_output = "";
    compiler_options = {
      escape_html: !argv['disable-html-escaping'],
    ...
    fs.stat(input_filename, function(err, stat) {
      ...
      if (!err) {
        if (!stat.isDirectory()) {
          ...
          compiled_output = CoffeeMaker.compileFile(input_filename, compiled_output, template_name, compiler_options);
          ...
          glob.glob("**/*.haml", "", function(err, matches) {
            ...
            if (!err) {
              for (_i = 0, _len = matches.length; _i < _len; _i++) {
                ...
                filename = "" + match;
                compiled_output = CoffeeMaker.compileFile(filename, compiled_output, null, compiler_options);
              }
              process.chdir(cwd);
              fs.writeFileSync(output_filename, compiled_output);
              process.exit(0);
