(function() {
  ...
  module.exports = Compiler = (function() {
    ...
    Compiler.prototype.node_factory = function(expression, current_block_level, current_code_block_level) {
      if (expression.match(/^(\/|-#)(.*)/)) {
        return new Comment(this.parent_node, expression, current_block_level, current_code_block_level, this.options.escape_html, this.options.format);
      } else if (expression.match(/^(-#|-|=|!=)\s*(.*)/)) {
        return new Code(this.parent_node, expression, current_block_level, current_code_block_level, this.options.escape_html);
      } else if (expression.match(/^(%|#|\.|\!)(.*)/)) {
        return new Haml(this.parent_node, expression, current_block_level, current_code_block_level, this.options.escape_html, this.options.format);
      } else if (this.parent_node instanceof Filter || expression.match(/^:(escaped|preserve|css|javascript|coffeescript)/)) {
        return new Filter(this.parent_node, expression, current_block_level, current_code_block_level, this.options.escape_html, this.options.format);
      } else {
        return new Text(this.parent_node, expression, current_block_level, current_code_block_level);
      }
    };
    ...
    Compiler.prototype.parse = function(source) {
      var attributes, expression, line, lines, result, whitespace, _results;
